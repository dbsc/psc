...
-- ∀ (N:Usize) (self : BigInt N) (other : BigInt N)(b1:Bool)(val1: BigInt (N))
-- (h1:N ≥ 1#usize )(h2: add_with_carry N self other = .ret (b1, val1)), (∃ (M:Usize) (self_reduit : arithmetics.BigInt (M)) (other_reduit : arithmetics.BigInt (M)) 
...
--   (b ↔ valImpl self.val + valImpl other.val + carry.val  > BigInt.max_value N) := by
--   sorry

def bool_to_int (b:Bool) := if b then 0 else (1 : Int)
...
--   ∃ b val, biginteger.BigIntegertestbigintegerBigIntN.add_with_carry N self other = .ret (b, val) ∧
